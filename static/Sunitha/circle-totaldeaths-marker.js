...
d3.csv("./Data/gun_data_07-16-Total.csv", function(error, shootingData) {
    ...
    shootingData.forEach(function(data) {
      ...
      function test (population) {
        ...
        ? Math.abs(Number(population)) / 1.0e+9 + "B"
        // Six Zeroes for Millions 
        : Math.abs(Number(population)) >= 1.0e+6
    
        ? Math.abs(Number(population)) / 1.0e+6 + "M"
        // Three Zeroes for Thousands
        : Math.abs(Number(population)) >= 1.0e+3
    
        ? Math.abs(Number(population)) / 1.0e+3 + "K"
    ...
    // alert(test(6800000)); // this outputs 6.8M
      ...
      var popup = '<b>County: </b>' + county +
                  ...
                  '<br/><b>Population: </b>'  + test(population) +
                  '<br/><b>Years: </b>' +  years;
